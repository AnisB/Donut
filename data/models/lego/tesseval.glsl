...
void interpolate()
{
  ...
  txCoord = (tx0 + tx1 + tx2);
  position = (p0 + p1 + p2);
  fnormal = (model*vec4(n0 + n1 + n2,0.0)).xyz;
}

void main()
{
  interpolate();
  gl_Position = modelviewprojection*vec4(position-texture(textureCmp,vec2((txCoord.x+2.0)/6.0,txCoord.y)).r*0.1*fnormal,1.0);
