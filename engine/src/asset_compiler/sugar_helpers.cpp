...
namespace donut
{
	...
	void HandleRenderableArray(TSugarDescriptor& sugar_descriptor, rapidxml::xml_node<>* renderable_list_node)
	{
        ...
        for (auto rend_node = renderable_list_node->first_node(RENDERABLE_NODE_TOKEN); rend_node; rend_node = rend_node->next_sibling(), ++renderable_idx)
        {
            ...
            current_renderable.id = rend_node->first_attribute(GEOMETRY_ID_NODE_TOKEN)->value();

			// Fetching the geometry
			rapidxml::xml_node<>* geometry = rend_node->first_node(GEOMETRY_NODE_TOKEN);
			assert(geometry);
			{
				current_renderable.geometry = geometry->first_attribute(GEOMETRY_LOCATION_TOKEN)->value();
			}
			...
			assert(material);
			{
				current_renderable.material = material->first_attribute(MATERIAL_NAME_TOKEN)->value();
			}
		}
	}

    bool read_sugar(const char* file_location, TSugarDescriptor& _sugar)
    {
        // reading the text file
		bento::Vector<char> buffer(*bento::common_allocator());
		bool res = bento::read_file(file_location, buffer, bento::FileType::Text);
		if(!res) return false;
