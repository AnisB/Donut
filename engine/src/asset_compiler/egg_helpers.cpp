...
namespace donut
{
	...
	{
		std::fstream in;
		in.open(target_path, std::fstream::in);
		if (!in)
		{
		...
		for (const auto& shape : shapes)
		{
			...
			if (nbInfo == 1)
			{
				output_egg._vert_normal_uvs.resize(8 * dimShape*nbShape);
				float* vertexArray = output_egg._vert_normal_uvs.begin();
				...
				// Creating the IBO
				output_egg._indexes.resize(verticeCounter / 3);
				for (uint32_t i = 0; i < verticeCounter / 3; i++)
				{
					output_egg._indexes[i] = {3 * i, 3 * 1 + 1, 3 * i + 2};
				}
			}
			else if (nbInfo == 2)
			{
